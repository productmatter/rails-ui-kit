...
module Ui
  module Select
    # One option model in the two shapes Select renders it. `native_options` goes through
    ...
    class OptionSet
      Item = Struct.new(:text, :value, :disabled, :selected, :group, keyword_init: true)
      ...
      def initialize(collection: nil, options: nil, model: nil, enum: nil, value_method: nil,
      ...
      attr_reader :selected_values, :disabled_values, :required
      ...
      def validate_source!
        raise ArgumentError, 'Ui::SelectComponent: enum: needs model:' if enum && model.nil?
        return unless collection && (value_method.nil? || text_method.nil?)

        raise ArgumentError, 'Ui::SelectComponent: collection: needs value_method: and text_method:'
      end

      def source_message(given)
        'Ui::SelectComponent needs exactly one option source — collection:, options: or enum: — ' \
      ...
      def placeholder(text)
        Item.new(text: text, value: '', disabled: false, selected: false, group: nil)
      end
      ...
      def collection_items
        return pairs_to_items(collection.map { |element| pair_from(element) }) unless group_method

        collection.flat_map do |group|
          label = call_on(group, group_label_method).to_s
          call_on(group, group_method).map { |element| item(*pair_from(element), group: label) }
        end
      end
      ...
      def item(text, value, group: nil)
        value = value.to_s
        Item.new(text: text.to_s, value: value, group: group,
