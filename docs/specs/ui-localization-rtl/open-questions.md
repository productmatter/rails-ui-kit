...
deadline: 2026-09-21
decided: (b), 2026-09-14, Jonathan Simmons — "convert now, promise later. Logical properties are just better CSS and carry no promise... converting 11 components costs hours; converting 40 later is a project." The sixteen classes and the guard build now; no README or docs claim; the native-RTL-reader gate stays deferred. This does not reverse the deferral `ui-positioning-and-navigation` and `ui-select` recorded — what stays deferred is the support claim, not the CSS. Overruled the default: (a)'s browser pass and README line are the promise, and the promise is what waits for a client.
