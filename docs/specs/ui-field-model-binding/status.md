...
ready-for-review. Ratified by the orchestrator on 2026-09-14. The help-text swap (§ Behavior, items 19–23) was added on the orchestrator's directive, relaying the decider, before the build began. Prerequisite 1 (Select's `aria-required`) landed under ui-select in `bb7e7b8`.
...
- Two additive prerequisites for `ui-choices` landed here on 2026-09-14, each backwards-compatible and each with its own test (`field_choices_test.rb`, `field_component_test.rb`): (1) a control component answers a class-level `labelable?`, defaulting to true on `Ui::Base` and false on `Ui::ChoicesComponent`, and `Ui::Field::LabelComponent` omits `for:` for a non-labelable control while keeping `id` — `for` has to name one labelable element, and pointing it at a group's first input would make clicking the label toggle that choice; (2) `Ui::Field::ControlComponent` hands a Choices control the record's value as `checked:`, the way it already hands Select `selected:`, unless the caller passed `checked:`. Input, Textarea and Select fields render exactly as before — provable — implementer
- This file's State stayed `building` after every agent-loopable check had gone green and this scope had been retired to the judgeable review and the three human gates already named in § In progress; corrected to `ready-for-review`, matching the pattern `ui-select` and `ui-presentational-components` use for the same situation — provable — implementer
- A third additive prerequisite for `ui-choices` landed here 2026-09-15, on the orchestrator's directive relaying an independent API review (item 2): `Ui::OptionSet` accepts a record for `model:`, coerced to `model.class` when the record doesn't answer the enum mapping but its class does -- `with_control(Ui::SelectComponent, model: @order, enum: :status)` inside a model-bound Field raised `NoMethodError` before this. Additively, `Ui::FieldComponent` gained `model_class` and `bound_attribute` (both nil without a model-bound Field), and `Ui::Field::ControlComponent` uses them so a control given `enum:` with no `model:` completes from the field's record class, and a Select or Choices control given no option source at all infers `enum:` from the bound attribute when it is one of the record class's `defined_enums` (checked with `respond_to?`, so a plain ActiveModel record infers nothing -- `test/support/test_order.rb` gained `defined_enums` and a `note` attribute as the enum/non-enum precedent). New test: `field_model_enum_inference_test.rb` — provable — implementer
