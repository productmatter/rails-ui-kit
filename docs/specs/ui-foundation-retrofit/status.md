...
- The scope still owned Toast and Confirm Dialog's tokens, `Ui::Base` adoption and API. Both moved to `ui-toast` and `ui-confirm-dialog` when the orchestrator ratified them on 2026-09-14. The spec, `implementation.md` and `open-questions.md` were trimmed to the four remaining components plus Confirm Dialog's overlay adoption, and the deletion of the nine class keywords is reversed there — tasteable — decider
