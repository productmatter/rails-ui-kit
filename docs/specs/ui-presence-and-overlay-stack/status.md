...
- `closeWithoutFocusReturn()`'s one-shot flag could outlive the close it was set for. It was cleared only in `restoreFocusIfLost()`, which runs when a close completes, but `hide()` has two ways out that never reach it: the overlay is already hidden when the requested close runs (a light-dismiss recovery got there first), and a reopen overtakes the exit animation so the overlay stays open. Either left the flag armed, and the next, unrelated close would have skipped returning focus to its trigger. Not live — only `ui--select` calls the method and nothing cancels its closes — but an unguarded edge in a shared primitive. Both exits now clear the flag, and `ui_overlay_focus_test.rb` pins the reopen case. Caught on review by the orchestrator, 2026-09-18 — provable — implementer
