...
- The chrome inventory grew by one on 2026-09-14: `rails_ui_kit.choices.required_message`, rendered by `Ui::ChoicesComponent` and read from the DOM by `ui--choices`, so `JS_FALLBACKS` doesn't grow (`ui-choices` § Behavior, item 14). 17 strings under 16 keys, by five components. The contract test and the Internationalization page absorbed it without special-casing; the counts stated as prose in § Intent and § Out of scope no longer name a number that a later scope has to chase — provable — implementer
