depends_on:
  - ui-select: owns the two chrome surfaces this scope changes most — the result-count announcement (its § Behavior, item 21) and the show-options button — and its rule 12 already requires every user-facing string to go through i18n
