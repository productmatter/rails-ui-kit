...
- § Behavior, item 17 said `size:` set each choice's minimum block size, so a one-line card would match the Input or Select beside it. It never did. A card is 12px of padding top and bottom, a 20px line and its border: 46px, already taller than the largest step's 40px minimum, so no step ever bound and the three sizes rendered identically. The acceptance check asserted only "at least the step's height", which 46px always satisfies, so nothing caught it; nor did the 2026-09-18 change that gave list rows a flat floor and described cards as "keeping" a height they never had. Jonathan caught it looking at the Sizes demo, and decided the same day to drop sizing from Choices: `size:` stays accepted and validated, reads no token, every choice takes the 24px floor, and the demo is gone. Caught by the decider — provable — implementer
