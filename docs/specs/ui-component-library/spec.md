...
| `ui-character-counter` | A character counter for `Ui::TextareaComponent` (`counter: true, limit: 500`): the count against a soft limit, rendered at the inline-end of Field's help-text line and giving way to the Field's error exactly as the description does; it counts what `validates length:` will count (code points, a line break as the two characters the browser submits), reads destructive past the limit and changes nothing else, and is announced only at thresholds. Passes rule 0's gate 1: it owns the length validator. Shaped and ratified 2026-09-18 from Jonathan's four decisions. | `ui-field-model-binding`, `ui-presentational-components`, `ui-localization`, `ui-localization-rtl`, `ui-test-harness` | ratified |

A future scope enters this table only after passing rule 0, pulled by a client build.
Select, Toast, Confirm Dialog, Choices and the character counter are the scopes in it; nothing is queued behind them.
