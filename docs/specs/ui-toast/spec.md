...
11. **Layout.** On tokens only, with logical properties only, and no `dark:` class in
    ...
    - **Footer** (`data-slot="toast-actions"`), present only when there are actions. It
      is a full-width row of the card below the icon, text and close button — not part
      of the text column — `flex flex-wrap justify-end gap-2`, so actions align to the
      card's inline-end padding edge and have the whole width (decided 2026-09-17: the
      text column crowded them). Actions wrap between one another. A label keeps
      Button's no-wrap, fixed-height contract. The root doesn't clip its content, so a
      label too long for the toast overflows visibly instead of being cut. Shortening
      it is the host's job, as it is for any Button. The docs recommend at most two
      actions — the close button already dismisses, so a "dismiss" action is a third
      control that only crowds the footer; the kit doesn't enforce the limit.
