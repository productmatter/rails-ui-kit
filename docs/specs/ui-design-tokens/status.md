...
- Jonathan's 2026-09-17 decision moves the palette from the warm hue-75 ramp and bespoke indigo to Tailwind's slate and indigo, every token a verbatim Tailwind v4 step, so a host reads the kit's defaults as Tailwind's own; the five places the kit steps away from Tailwind's usual pick are each a contrast contract, below — tasteable — decider
- With the slate/indigo move the 2026-09-13 `--input` and `--destructive` values above are superseded: `--input` is slate-500 light / slate-400 dark (Tailwind's slate-300 input border is ~2.6:1), computed 4.35–4.77:1 light and 5.58–7.68:1 dark against every surface and the `dark:bg-muted/50` fill; light `--destructive` is red-700, not red-600, which `field_test.rb` measured at 4.35:1 as text on `--muted` after the move — red-700 is 5.86–6.42:1 on every light surface and 6.42:1 under a white label; dark `--destructive` is red-400 with a slate-950 label, 5.07–6.97:1; dark `--primary` is indigo-400 with the same dark label (indigo-500 is 3.2–4.4:1 as link text); light `--muted-foreground` is slate-600, not slate-500, which is 4.35:1 on `--muted`. Ratios computed oklch → sRGB, then held in the browser by `field_test.rb`, `tokens_test.rb` and the per-component checks — provable — implementer
- Dark `--border`, and `--sidebar-border` which mirrors it, moved from slate-800 to slate-700 on Jonathan's 2026-09-18 decision. The same token has a different job in each mode: in light mode a shadow separates a floating surface and the border is decoration, but in dark mode the shadow is invisible and the border is the only edge there is — and slate-800 on `--popover` was 1.22:1, so the confirm dialog's outline was present and could not be seen. slate-700 is 1.73:1 on `--popover` and 1.95:1 on `--background`. With it, `Ui::ModalComponent` moved from `bg-background` with no edge to `bg-popover` with `border border-border`: the fill and edge every other floating surface (Toast, Popover, Dropdown, Select's popup, Confirm Dialog) already had, and the docs' hand-written dialogs follow it. Light mode is unchanged — tasteable — decider
- Dark `--input` moved from slate-400 to slate-500 on Jonathan's 2026-09-18 decision: slate-400 cleared the 3:1 boundary rule by so much (5.58–7.68:1) that a field's border read as nearly white. slate-500 is the step light mode already uses, and computed against every dark surface and the `dark:bg-muted/50` fill composited over it, it is 4.23:1 on `--background`, 3.74:1 on `--card`/`--popover` and 3.07:1 at the tightest, a field on `--muted`. This supersedes the dark value in the 2026-09-17 entry above — tasteable — decider
