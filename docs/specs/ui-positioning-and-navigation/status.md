...
- § Behavior, Primitive E said the install generator writes an identity `field_error_proc` into the host's initializer, and the judgeable and human-gate checks assumed that step. `install_generator.rb` never did it, and Field doesn't need it, because its controls never pass through Rails' `Tags` classes, where the proc fires. Setting the proc in a host app is a Rails primitive override, so on the orchestrator's ruling (2026-09-14) it is deferred with the form builder rather than built. Primitive E, Critical files, the judgeable check and § Out of scope / deferred now say so, and the human gate that asked Jonathan to approve the step is removed; see `ui-field-model-binding` § Out of scope / deferred — tasteable — reviewer
...
- § Behavior, Primitive F (`ui--media-query`) is removed, on the decider's ruling of 2026-09-14: "the components themselves, if they have a dependency like the select example, should just have their own CSS media queries built into the component HTML such that it automatically swaps using the browser." It was built for Sidebar and Drawer, which were cut against rule 0 and never existed, and its only consumer was Select. `media_query_controller.js`, its registration, `test/system/media_query_test.rb`, the Media Query docs page and this scope's two acceptance checks for it are deleted; it was never in a released version, so no host can be consuming it. Select now swaps in its own CSS (`pointer-coarse:` / `not-pointer-coarse:`, compiled and checked in the browser) and flips the select's `aria-hidden` and `tabindex` from a `matchMedia` listener in `select_controller.js`. `ui-component-library` § Business rules, rule 4 no longer names media-query watching a shared primitive — tasteable — decider
